...
def test_gateway_panel_compact():
    state = DashboardState(
        gateway=GatewayState(
            pid=12345, running=True, state="running",
            platforms=[
...
def test_gateway_panel_detail():
    state = DashboardState(
        gateway=GatewayState(
            pid=12345, running=True, state="running",
            platforms=[
                PlatformStatus(name="telegram", state="connected", updated_at="2026-04-08T17:42:57+00:00"),
            ],
        ),
    )
...
def test_sessions_panel_compact():
    state = DashboardState(
        sessions=[
            SessionInfo(session_id="sess_001", source="cli", message_count=77, tool_call_count=51, is_active=False),
...
def test_tools_panel_compact():
    state = DashboardState(
        tool_stats=[ToolStats(name="shell_exec", call_count=23), ToolStats(name="web_search", call_count=18)],
        total_tool_calls=89,
        available_tools=29,
    )
...
def test_config_panel_compact():
    state = DashboardState(
        config=ConfigSummary(model="gpt-5.4", provider="openai-codex", personality="kawaii", max_turns=192),
    )
...
def test_overview_panel_compact():
    state = DashboardState(
        skills_memory=SkillsMemory(
            skill_count=70, skill_categories=28, memory_file_count=0,
...
def test_logs_panel_compact():
    state = DashboardState(
        logs=LogState(agent_lines=[LogLine(timestamp="15:42:03", level="INFO", message="Session saved")]),
    )
