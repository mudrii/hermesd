...
- Snapshot export (`--snapshot-file`) now writes to a temporary file that is atomically renamed into place: a destination hard-linked to a file inside `~/.hermes/` can no longer truncate the protected file, and an interrupted write never leaves partial output behind.
...
- Cron 24-hour counters and per-job last-run status are now computed with full-window SQL aggregation instead of counting rows from the 500-row display history, so busy installations no longer undercount and chatty jobs can no longer crowd out other jobs' statistics.
...
- Sessions detail view is now scrollable (`j`/`k`, `g`/`G`) like the Logs and Skills detail views, so the sessions table and lower sections remain reachable at ordinary terminal heights.
- Log tail reads are strictly bounded to `--log-tail-bytes` even when the file grows between the size check and the read.
...
- `state.db` in WAL mode is now opened live and read-only whenever its `-shm` sidecar exists — WAL readers see consistent data without blocking the writer — instead of copying the whole database into a temp dir on every refresh. Live reads use SQLite's `readonly_shm=1` mode (SQLite ≥ 3.50), so hermesd no longer updates wal-index read-marks in `state.db-shm` and the literal zero-write guarantee holds; on older SQLite, or when the `-shm` is missing or the live open fails, hermesd falls back to the full snapshot copy. The `-wal` probe now uses strict existence checking so an unreadable sidecar fails the source instead of silently serving checkpoint-lagging data via `immutable=1`.
