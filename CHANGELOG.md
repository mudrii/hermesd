...
- URL redaction is now scheme-agnostic and case-insensitive: uppercase schemes (`url=HTTPS://user:pw@host/x`) and non-HTTP schemes (`ftp://`, `ssh://`) have their credentials masked in `key=<url>` argv forms and in free text, not just lowercase `http(s)` URLs.
