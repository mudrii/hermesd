...
| # | Panel | What It Shows |
|---|-------|---------------|
| 1 | **Gateway & Platforms** | Live gateway PID, Hermes version, update status, drain/scale-to-zero state, served-profile record, per-platform profile routing and recorded shared-listener ingress URL, loop-tick witness verdict, crash forensics, respawn-storm counts, web-client attachment, recorded per-profile mirror URLs, multiplex-migration verdict and standalone reason, degraded state and watchdog exits, heartbeat memory pressure, dead delivery targets, restart-loop breaker, restart/update backlog, backend heartbeats by profile and host, channel aliases/staleness, platform families, per-platform connection dots, and channel-directory inventory |
| 2 | **Sessions** | Active/total count, identity-verified live surfaces, display names, pinned sessions, branch/profile, last-activity age, compression-failure warnings, turn leases/compression locks, hygiene cooldowns, decoded chat routes, reset churn, CLI terminals, active compression-recovery state (live failure cooldown, armed anti-thrash probe deadline, fallback streak, ineffective-compaction strikes), message/tool/API call totals, cwd, archived state, handoff metadata, parent-session lineage, per-repository session counts, hour-of-day activity, and bot transport profile |
| 3 | **Tokens / Cost** | Today's and all-time token usage, per-model usage from `session_model_usage` (all-time/24h/7d) with actual vs estimated cost, an auxiliary-work subtotal, cost-status reconciliation, recent-window rollups, provider/endpoint breakdowns, 14-day daily usage, per-source 24h/7d usage, and the costliest 7d sessions |
| 4 | **Tools** | Available tools count, toolset availability (enabled/unavailable/lazy/disabled), per-session call stats, background processes with purpose/port/profile, dead-pid, reused-pid and orphaned-helper markers, filesystem checkpoints, full tool name grid |
| 5 | **Config** | Point-in-time `backups/config/` snapshots, model, provider, personality (`display.personality`), MoA, Tool Search, dashboard auth, kanban, code execution, gateway, routing and memory/session settings, agent limits (delegation concurrency/depth/orchestrator, goal turn budget, tool-loop guardrails, live sessions, streaming, logging) integrations (MCP, plugins, updates, proxy presence), and file-only doctor findings |
| 6 | **Cron** | Scheduler tick/provider, ticker health, recorded ticker error, missed-run catch-up policy and counter, open and resolved incidents, Chronos config presence, suggestion count, job table with schedule, effective model, quota holds, delivery target, 24 h run counters, error count, latest error, and output metadata, recent failures, per-job token usage, the delivery queue, deferred Bot Chat receipts, and recovery ledgers |
| 7 | **Skills / Integrations** | Provider auth status/freshness, credential pools with pool and per-model cooldowns, provider throttling, an integrations inventory (pairing, webhooks, profile routes, monitoring, telemetry), Skills Hub counts, hooks, separate agent/Desktop plugin inventories, agent-plugin configured-activation gate, catalog drift/removal checks, Nous free-tier badge, plugin manifest format and precedence conflicts, plugin install/catalog provenance with `--ref` drift, declared version gate and capabilities, MCP inventory and schema-cache validity, prompted-skill snapshot, BOOT.md presence, skills with descriptions |
| 8 | **Logs** | Tailed agent, gateway, errors, cron, desktop, dashboard, dashboard error/restart, GUI, update, gateway-error, crash, skills-hub and dashboard-auth audit, MCP-stderr, and workspace logs with Tab switching, inline filtering, a per-stream scope label (root vs. profile), and health counters for the MCP, gateway-error and workspace logs |
| 9 | **Profiles** | Read-only profile discovery with session counts, log freshness, skill counts, DB size, `config.yaml`/`.env` presence, platform credential names shared across profiles, and SOUL excerpts |
| 10 | **Memory** | Memory provider, MEMORY.md/USER.md word counts, learning summary, SOUL.md size/excerpt, and memory file inventory |
| 11 | **Kanban** | Read-only kanban task/run/event/comment counts, multi-board summaries, stale claims, dispatch config, active workers with reused-pid checks, blocked/failing tasks, completion contracts with breaker state, notify-subscription backlog, and recent runs |
| 12 | **Operations** | Dashboard process count, Desktop and Web UI build stamps, Response Store, verification evidence, goals, async delegations with handoff counts, live delegation transcripts, process receipts, checkpoint-prune and corrupt-ledger markers, state.db maintenance, state snapshots with manifest verdicts, ESTOP state, disk and retention, log health, staged writes pending review, MoA trace metadata, Projects/correlations/newest repos, model-cache summaries, PR monitor state (including open/conflicting PR counts), blocked-script counts, hosted-room coordination counts, and retained API run reservations |
| 13 | **Curator** | Skill hygiene from `skills/.usage.json` (patch-reuse backlog, state counts, threshold windows) plus scheduler state and the newest memory-curation run: skill before/after counts, archived/pruned/added totals, model/provider, duration, tool-call total + per-tool breakdown, state-transition trail, LLM summary or error, suppressed built-ins, the recent curator ledger, and the live run claim |
...
- **Header status** — the top-left header shows the installed `hermesd` version, while the header/footer surface an `AGENT OFFLINE` warning when Hermes Agent appears inactive and a `⏸ PAUSED (ESTOP)` banner while `hermes pause` is engaged
...
**Multiplex mode and migration.** `gateway.multiplex_profiles` is read the way upstream reads it: a top-level `multiplex_profiles` wins when present, string tokens are parsed, and an unset key means the default (on) that the gateway settles at boot. An explicit `false` is retired upstream and ignored, and the panel says so instead of presenting it as an opt-out. When the gateway recorded a `multiplex_standalone_reason` in `gateway_state.json`, the detail header shows it and the compact view warns `⚠ standalone (not multiplexing)`; the single-profile reason (nothing to multiplex) does not warn. `gateway_migration.json` is deleted by upstream once an apply confirms convergence, so a manifest on disk means the migration is **unfinished**: the **Multiplex Migration** section shows `⚠ migration unfinished`, names the missing evidence, and says to resume with `hermes gateway migrate --multiplex`. No manifest cannot tell "never migrated" from "converged" or "rolled back", and is rendered as `no migration manifest recorded (none pending)`. The section keeps three things apart: the *recorded intent* (`migrated_at`, always labelled `Started:` because it is when the attempt began, plus the recorded `flag_was`, homes and service managers), *intermediate progress* (default gateway live? a live served record? which recorded profiles does it cover?), and one *verified current topology* verdict. That verdict is `● multiplexed (verified)` only when the manifest has a valid supported version-1 schema, the default gateway source is fresh and live, and its current `served_profiles` covers `default` plus every secondary the manifest recorded; the verdict then notes that the manifest is still on disk. If the base gateway source fails, migration is marked degraded and keeps its own last-good verdict instead of checking a newly written manifest against cached topology. The panel never says "migrated". The manifest's recorded `home` values are display data only and are never used to build a path hermesd reads. Upstream verifies against every profile in its *plan*, which includes profiles that never had a standalone gateway and so are absent from the manifest: hermesd can only see the manifest, so its expected set is a subset of upstream's.
...
**Liveness, lifecycle and updates.** A live gateway in state `degraded` (a configured platform is parked or retrying) counts as serving, as upstream counts it, and renders as `Degraded` rather than stopped; a dead record whose exit reason is `loop_liveness_watchdog` or `shutdown_watchdog` is reported as a watchdog exit. The compact view adds a `loop:` indicator next to the running dot, using the loop-tick witness and heartbeat evidence described below. Legacy and unverifiable records remain distinguishable, and a fresh heartbeat alone does not establish responsiveness. One-line warnings, shown directly under the status line, cover configuration changes, unfinished updates, code skew, an unfinished migration, standalone mode, memory pressure, dead delivery targets, a tripped restart-loop breaker, owed restarts, and pending/failed deliveries. Platforms flagged `needs_attention` get a `!` marker.

The detail view adds a **Liveness** section (heartbeat age, incarnation count, restarts in the last 24 h, current incarnation uptime, running code version/short sha, config generation, session-store status), the gateway lifecycle (phase, last exit code/reason, and a warning when the previous life ended without recording an exit), an **Updates** section (outcome, finish age, from → to version, first failed step, the recorded post-restart fleet matrix as state counts, the post-swap pid, owed manual serve restarts, runtime outcomes and skips, the ten newest archived receipts with the runs that never finished cleanly, and the runtime code-skew verdict with the evidence behind it; fleet rows in state `external` serve a checkout the update did not touch, so they are listed by code root and never counted as skew), the **Shared-Listener Ingress** and **Multiplex Migration** sections described above, and a **Delivery Obligations** table with the five newest undelivered messages (platform, state, attempts, age, truncated last error — message content is never read). The platform table gains a **Retrying** age column and a `! needs attention` status marker.

**Memory, dead targets and restart backlog.** The heartbeat's memory sample (Linux gateways only: RSS, available and total memory, swap) is shown with upstream's pressure tier, which reads `unknown` once the sample is older than 150 s; the compact view warns at `elevated` or `critical`. `gateway/dead_targets.json` is reported as a count, per-platform counts and the newest three targets with a redacted reason — chats delivery stopped sending to until a send succeeds again; chat ids are never read. The restart-loop breaker (`gateway/restart_loop.json`) is evaluated against the `gateway.restart_loop_guard` policy of root `config.yaml`, and a tripped breaker, which makes the next restart-interrupted boot skip auto-resume, is a warning. A **Restart Backlog** section shows the back-online notice a planned restart still owes home channels (`.restart_pending.json`, with its age, service/detached flags and delivered count) and the manual serve restarts still owed after an update (`serve_restart_pending/*.json`; a record whose process is provably gone is counted as stale). **Backends** groups the `state.db` `gateway_heartbeats` rows by profile and host (up to eight groups), with each group's row count, whether its newest beat is live, and the beat age.
...
A **Doctor (file checks)** row reports the raw-file drift `hermes doctor` warns about: stale root `provider`/`base_url` keys that belong under `model:`, and legacy `custom_providers` entries without a `providers:` twin. The compact view shows the finding count.
...
Each job shows the model its next fire resolves to and where it came from — `pinned` (the job's own `model`), `cron.model default`, or `follows main model` — using upstream's fire-time precedence; the retired `model_snapshot`/`provider_snapshot` keys are not read. A job parked by a provider quota hold shows `held until X (provider usage window)` and a compact `held` marker; an expired or unparseable hold is inert, as upstream treats it. The job rows also surface the newer `cron/jobs.json` keys: `failure_streak` (shown as `✗3` in the compact row), `paused_at`/`paused_reason` (`⏸` — either one alone is enough to mark a job paused), `last_delivery_error`, `last_dispatch` lateness and kind, `repeat` progress, and `no_agent` script-only jobs.

**Token usage, deliveries and recoveries.** Four more `cron/` stores each have their own health source and last-good fallback. **Token Usage** reads a 1 MiB tail of `cron/usage_audit.jsonl` (one line per fire, never pruned upstream) into per-job 24h/7d fires and tokens plus the last fire's tokens, model, duration and redacted error; when the tail ends inside the 7d window, the 7d figures are marked as a lower bound. **Delivery Queue** reads `cron/deliveries.db`, the sends a cron worker queued for a live gateway: retained status counts, the pending backlog with its oldest age, failed or unknown sends in the last 24h, and the newest failures with redacted errors; payload columns are never read. **Deferred Bot Chat** scans `cron/bot_chat_pending/`, cron outputs deferred behind a Bot Chat owner: counts by status, the unsettled backlog with its oldest age, and claimed or ambiguous receipts with redacted errors; receipt content is never kept. **Recovery Ledgers** reads capped tails of `persisted_error_recoveries.jsonl`, `timezone_migration_catchups.jsonl` and `inflight_forced_releases.jsonl` into 24h/7d counts and the newest entry's job and detail. The compact view adds one warning line each for waiting or failed deliveries, unsettled Bot Chat sends, and recoveries in the last 24h.
...
Press `7` for provider and integration visibility in one place: **Providers** with active auth state and safely persisted credential freshness/expiry metadata, **Credential Pools** with redacted metadata and the remaining cooldown of an exhausted credential (until `last_error_reset_at`, else `last_status_at` plus the TTL for the recorded error, as upstream computes it), a **Model cooldowns** list of models a pooled credential is benched for, **Provider Throttling** from active `rate_limits/nous*.json` holds with time remaining, **Skills Hub** installed and quarantined counts from `skills/.hub/`, **Hooks** discovered from `~/.hermes/hooks/`, agent **Plugins** from `~/.hermes/plugins/`, app-level **Desktop Plugins** from `~/.hermes/desktop-plugins/`, **MCP Servers** from `config.yaml` with secret-bearing args and URL query params redacted, `BOOT.md` presence, and **Skills** grouped by category with descriptions loaded from each skill's `SKILL.md` frontmatter. Use `j`/`k` to scroll through the full detail view.
...
Press `8` for the full log viewer with discovered streams such as **agent**, **gateway**, **errors**, **cron**, **desktop**, **dashboard**, **dashboard.error**, **dashboard.restart**, **gui**, **update**, **gateway.error**, **tui crash**, **skills.audit** (`skills/.hub/audit.log`), **auth.audit** (`logs/dashboard-auth.log`), **mcp.stderr**, **workspace**, and **workspace.error** (each shown only when its file exists). The tab bar wraps to the terminal width. Press `Tab` to switch between them, `/` to filter the current log stream by `level:`, `minlevel:`, `component:`, `session:`, or free text, `j`/`k` to move the viewport, and `g`/`G` to jump to the top or bottom.

The stream list is not one directory: **agent**, **gateway**, **errors**, **dashboard.restart** and **auth.audit** come from the selected profile's `logs/` and **skills.audit** from its `skills/.hub/`, while the rest come from `~/.hermes/logs/` (and **cron** from `~/.hermes/cron/output/`). Since a stream's file name alone cannot tell `profiles/coding/logs/agent.log` apart from `logs/agent.log`, the detail view prints a `Scope: profile` / `Scope: root` line under the tab bar for the stream you are reading, and `--snapshot-format json` carries the same value as `logs.streams[].scope`. The label names the resolver that *owns* the stream, not the directory it happened to resolve to — with no profile selected everything lives under `~/.hermes/`, but the profile-owned streams still read `profile`. Which scope owns every other source is recorded in [`.codex/rules/source-ownership.md`](.codex/rules/source-ownership.md).
...
**What does `PAUSED (ESTOP)` mean?**
`hermes pause` wrote an `ESTOP` sentinel: cron, the kanban dispatcher and new gateway turns skip new work, while in-flight work keeps running. The header shows the recorded reason and age until `hermes resume` removes the sentinel.
...
hermesd/
  ...
  collect/        Per-domain readers behind the collector facade
    ...
    config.py     config.yaml and auth.json summaries, credential cooldowns,
                  profile routes, doctor file checks
    cron.py       Cron output discovery, excerpts, suggestions,
                  executions.db history/incidents, ticker health, usage audit,
                  delivery queue, Bot Chat receipts, recovery ledgers
    curator.py    Curator run reports, scheduler state, thresholds, skill hygiene
    desktop_plugins.py
                  Content-free app-level Desktop plugin inventory
    gateway.py    Gateway heartbeat, lifecycle, updates, ledgers, memory,
                  dead targets, restart loop and restart backlog
    hosted_rooms.py
    ...
    integrations.py
                  Pairing, webhook subscriptions, shared metrics and rate-limit
                  holds (names, counts and timestamps only)
    kanban.py     Kanban board SQL readers and board discovery
    logs.py       Log line parsing and incremental log-health counters
    ...
    operations.py Verification, goals, projects, MoA, delegations, receipts,
                  state snapshots, disk usage, pending actions
    ...
    sessions.py   Session, token and cost analytics, daily/source/repo usage
    skills.py     Skill, memory and SOUL filesystem readers
    sqlite_util.py Read-only SQLite helpers
    system.py     Process liveness and identity, git checkpoints, activity age,
                  ESTOP sentinel
