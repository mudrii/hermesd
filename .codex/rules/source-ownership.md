...
| `migration` | ROOT | `migration` | `gateway_migration.json`, `config.yaml` | `shared_path` | `<default home>/gateway_migration.json` — `hermes_cli/gateway_migrate.py:25` (`MANIFEST_NAME`) + `:467-468` (`_manifest_path`), where the home is `_default_home()` = `get_default_hermes_root()` at `:144-146`, never a secondary's; `_read_multiplex_flag` reads `default_home/"config.yaml"` — `hermes_cli/gateway_migrate.py:203-215` | agrees (`gateway_migration.json`); diverges — **intentional** (`config.yaml`, same decision as `config`) | `test_profiled_collector_reads_the_root_migration_manifest`, `test_profiled_collector_ignores_a_profile_local_migration_manifest` |
