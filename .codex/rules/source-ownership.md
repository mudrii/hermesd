...
| `generation_churn` | PROFILE | `session_coordination.generations`, `session_coordination.generation_*` | `state.db` (`conversation_generations`) | `profile_path` | `get_hermes_home()/"state.db"` — `hermes_state.py:160,178`; writer `hermes_state_common.py:153,482-487` | agrees | `test_profiled_collector_reads_session_coordination_from_the_profile_db` |
...
| `~/.hermes/state/gateway.loop-tick.<pid>.sock` | PROFILE — `gateway/shutdown_watchdog.py:169` | Loop-scheduling witness of the root gateway. Probed read-only: the client sends nothing, reads at most one byte (`hermes_cli/gateway.py:363-376`). |
| `~/.hermes/gateway-starts.log` | PROFILE — `gateway/status.py:57-83` | Respawn-storm ledger for the root gateway only; kept beside the other launch-home gateway files so the storm count and the heartbeat that clocks restarts describe the same process. |
| `~/.hermes/state/dashboard_clients.heartbeat` | PROFILE — `gateway/scale_to_zero.py:97-111` | Attachment marker of the web dashboard served by the root gateway; stat'd for its mtime, never read. |
| `~/.hermes/logs/gateway-exit-diag.log` | PROFILE — `hermes_cli/gateway.py:4643-4665`, `gateway/lifecycle_ledger.py:96-104` | Crash forensics of the root gateway's exit paths (plus its stat-only companions: shutdown-diag, faulthandler, launchd-reload). Tail-read with the log-tail-bytes cap; extras such as tracebacks and argv are never parsed. |
...
| `~/.hermes/cache/delegation/live/` | PROFILE — `tools/delegation_live_log.py:40-43` ("profile-safe, never ~/.hermes") | Feeds the live-log count inside `operations` and the whole `delegation_live` source (manifests + task-log tails); kept on the root resolver so both read the same home. |
...
| `~/.hermes/cache/plugin-catalog.json` | PROFILE — `hermes_cli/plugin_catalog.py:216-218` | The live-catalog cache hermesd compares the plugin sidecars against. Kept on the root resolver with the `plugins/` row above so both sides of the drift comparison describe one home: comparing a root-installed plugin's sidecar against a profile's cache would manufacture drift. Behaviour pin: `test_plugin_catalog_cache_is_read_from_the_shared_root_under_a_profile`. |
...
| `OperationsState` | Profile: `projects*`, `verification_*`, goals/delegations/`state_db_*`, `db_recovery`, **`api_runs`**. Root: `response_store_*`, `moa_trace_*`, `snapshot_*`, `blocked_script_*`, `delegation_live_log_count`, both build stamps, `pr_monitors`, **`hosted_rooms`**. The last two are the sharpest case in this table: they are rendered in one section of panel 12, one after the other, and resolve to different homes. |
