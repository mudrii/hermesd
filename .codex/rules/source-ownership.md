...
| `update_receipt` | ROOT | `gateway.last_update_*`, `gateway.runtime_code_skew*`, `gateway.update_*` (fleet, post-swap pid, owed manual serves, settle stamp, runtime outcomes, skips) | `logs/update_receipts/latest.json` | `shared_path` | `hermes_cli/update_receipt.py:120-122` | diverges | UNPINNED |
...
| `restart_loop` | MIXED | `gateway.restart_loop_*` | profile: `gateway/restart_loop.json`; root: `config.yaml` (`gateway.restart_loop_guard` policy, same decision as `config`) | both | `get_hermes_home()/"gateway"/"restart_loop.json"` — `gateway/restart_loop_guard.py:36-37`; policy `gateway/run_shutdown.py:344-363` | agrees (`restart_loop.json`); diverges — **intentional** (`config.yaml`) | `test_restart_loop_is_read_from_the_selected_profile` |
...
| `update_receipt_history` | ROOT | `gateway.update_history_*`, `gateway.update_failures` | `logs/update_receipts/update_*.json` | `shared_path` | `hermes_cli/update_receipt.py:120-122` (dir), `:206-211` (archive), `:241-246` (prune to 20) | diverges | UNPINNED |
...
| `process_identity` | ROOT | `background_processes[].identity`, `background_processes[].spawner_pid`, `background_processes[].orphaned` | `spawn-ledger.json` (the cached read `background_processes` already made) plus a process start-time probe | `shared_path` | `get_default_hermes_root()/LEDGER_FILENAME` — `hermes_cli/process_identity.py:27,127-131`; `(pid, create_time)` identity `:174-191`; spawner recorded `:280-302`; orphan verdict `spawner_is_dead` `:325-332`, reaped `:335-360` | agrees | UNPINNED |
...
| `state_snapshots` | ROOT | `operations.snapshot_count`, `operations.snapshot_total_bytes`, `operations.newest_snapshot_age_seconds`, `operations.snapshots`, `operations.snapshot_failed_count` | `state-snapshots/` (bounded recursive walk, loose `-wal`/`-shm`/`-journal` sidecars grouped with their db), `state-snapshots/<id>/manifest.json` | `shared_path` | `home/"state-snapshots"` where `home = get_hermes_home()` — `hermes_cli/backup.py:35,1088-1090`; "each lands under its OWN `<home>/state-snapshots/`" `hermes_cli/backup.py:1380`; manifest written `hermes_cli/backup.py:1271-1276` | diverges | UNPINNED |
...
| `shared_metrics` | PROFILE | `integrations.shared_metrics_*` | `telemetry/shared_metrics/metrics.sqlite3` (`counter_aggregates`, `package_outbox` counts by `send_state`, rows with a `last_error`, `consent_marks`; payloads and error text are never read) | `profile_path` | `get_hermes_home()/"telemetry"/"shared_metrics"` — `hermes_cli/observability/shared_metrics.py:171-176` | agrees | UNPINNED |
...
| `~/.hermes/cron/persisted_error_recoveries.jsonl`, `cron/timezone_migration_catchups.jsonl`, `cron/inflight_forced_releases.jsonl` | PROFILE — `cron/jobs.py:1011-1023` (`_current_cron_store()`), `cron/scheduler.py:868-884` | Same `cron/` store as `jobs.json`; must move with it. Never pruned upstream, so only capped tails are read. |
...
| `~/.hermes/logs/update_receipts/latest.json`, `update_*.json` | PROFILE — `hermes_cli/update_receipt.py:120-122` | Update/code-skew verdicts and the archived failure history describe the root home's updates only; kept together so latest and history never describe different homes. |
...
| `RuntimeStatus` | `agent_running` / `last_activity_age_seconds` come from profile `state.db`, `sessions/sessions.json` and `logs/agent.log`, but also from root `gateway_state.json` (`collect/system.py:227-237`). A "runtime idle" banner can therefore mix one profile's activity with the root gateway's. The `estop_*` fields (source `estop`) come from whichever `ESTOP` sentinel was found first — profile, then root — and `estop_scope` names it. |
